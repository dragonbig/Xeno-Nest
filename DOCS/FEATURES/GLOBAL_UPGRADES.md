...
### 4. NEST_SHIELD — 둥지 보호막 (🛡️)

**효과:** NEST가 받는 모든 피해를 감소시킨다.

**적용 공식:**
```
dmg = Math.round(originalDmg × (1 - shieldLv × 0.015))
```

두 곳에서 적용된다:
1. `pursueTarget()` — 근접 공격이 NEST를 타격할 때
2. `updateEnemyProjectiles()` — 원거리 투사체가 NEST에 명중할 때

**레벨별 피해 감소:**

| NEST_SHIELD Lv | 피해 감소율 |
|----------------|-----------|
| Lv.1 | 1.5% |
| Lv.10 | 15.0% |
| Lv.20 | 30.0% |
...
`openBuildingPanel()`에서 NEST 선택 시 6종 업그레이드 버튼이 순서대로 생성된다.
...
6종 모두 30레벨이며, 비용은 기하급수적으로 증가한다. 대략적인 증가 비율(`cost[i+1] / cost[i]`)은 약 1.325배이다.

| 업그레이드 | Lv.1 비용 | Lv.30 비용 | 총합 (30레벨 도달) |
|-----------|----------|-----------|------------------|
| SELF_REPAIR | 150 | 553,500 | 약 165만 |
| THORN_BOOST | 120 | 417,940 | 약 125만 |
| SPORE_BOOST | 130 | 460,545 | 약 137만 |
| NEST_SHIELD | 200 | 733,000 | 약 219만 |
| RESOURCE_BOOST | 100 | 355,575 | 약 106만 |
| WALL_FORTIFY | 120 | 421,913 | 약 126만 |

NEST_SHIELD가 가장 비싸고, RESOURCE_BOOST가 가장 저렴하다. RESOURCE_BOOST를 먼저 올려 자원 수급을 강화한 뒤 나머지를 올리는 전략이 경제적으로 유리하다.

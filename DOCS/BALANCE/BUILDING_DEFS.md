...
> **카테고리:** BALANCE
> **최초 작성:** 2026-03-23
> **최종 갱신:** 2026-03-24 (Phase 8: BALLISTA 타워 신규 추가)
...
Phase 6 변경:
- RESOURCE_STATS.amount: 15 → 7 → 14 (최종 14). 자원 생산량 밸런스 조정을 거쳐 기존 대비 약간 감소한 14로 확정.
- NEST_BUILD_CAP: [10, 20, 50] → [5, 10, 25] (50% 감소). 건물 배치 수 제한 강화.
- THORN 투사체 속도 2배(200 → 400) + 유도(homing) 추가.
- SPORE 범위 피해(AoE) splashRadius 1.2타일, 50% 피해 + 슬로우 추가.
- NEST 자체 자원 생산 추가 (8초마다 Lv.1=5, Lv.2=10, Lv.3=20).
- NEST_SHIELD 제거 → WALL_DEFENSE(방벽 피해 감소 2%/lv) 교체.
- 글로벌 업그레이드 6종 비용 통일 (시작 100, x1.18, Lv.30=12,150).

Phase 8 변경:
- BALLISTA(발리스타) 공격 타워 신규 추가. 아이콘 🏹, 비용 80, 건설 시간 4s, HP 90.
- 타겟팅 우선순위: ARCHER → MAGE → 일반 적. PHYSICAL 공격 속성.
- 타일 타입 `TILE.BALLISTA = 10` 신규 추가.

---

## 건물 7종 정의 (BUILDING_DEFS)
...
- PLACING 상태에서 NEST_ZONE(colMin 14, colMax 15, rowMin 3, rowMax 4) 내 보라색 타일 4칸 중 어디를 터치해도 좌상단 고정 좌표(14, 3)에 자동 배치된다.
...
NEST 건물이 건설 완료되면 자체적으로 자원을 생산한다. 업그레이드 중에는 생산하지 않는다.

| 상수 | 값 |
|------|----|
| NEST_RESOURCE_INTERVAL | 8초 |
| NEST_RESOURCE_AMOUNT | [5, 10, 20] (Lv.1=5, Lv.2=10, Lv.3=20) |

광고 버프(G.adBuff) 활성 시 NEST 자원 생산량도 2배가 된다.
...
| 투사체 속도 (px/s) | 400 | 400 | 400 | 400 | 400 |
| 유도 (homing) | O | O | O | O | O |
...
- Phase 6에서 투사체 속도가 200 → 400으로 2배 증가하고, 유도(homing) 기능이 추가되었다. 투사체가 매 프레임 타겟 방향으로 속도 벡터를 갱신하므로 이동 중인 적도 정확히 타격한다.
...
**범위 피해 (AoE, Phase 6 추가):**

| 항목 | 값 |
|------|----|
| splashRadius | 1.2 타일 (57.6px) |
| 범위 피해량 | 직격 피해의 50% |
| 범위 슬로우 | 직격과 동일 (30% 감속, 3초) |

SPORE 투사체가 타겟에 명중하면 splashRadius 내 모든 적에게 50% 피해와 슬로우를 부여한다. 직격 대상은 100% 피해를 받는다.
...
### BALLISTA — 발리스타 (Phase 8 신규)

```
cost: 80      buildTime: 4s
hpMax: 90     armorType: STRUCTURE
icon: 🏹
tile: TILE.BALLISTA = 10
```

**공격 스탯 (BALLISTA_STATS) — 5레벨 배열 (인덱스 = 레벨 - 1):**

| 항목 | Lv.1 | Lv.2 | Lv.3 | Lv.4 | Lv.5 |
|------|------|------|------|------|------|
| 사거리 (타일) | 5.0 | 5.5 | 6.0 | 6.5 | 7.0 |
| 공격력 | 55 | 80 | 110 | 145 | 190 |
| 발사 속도 (/s) | 0.5 | 0.55 | 0.6 | 0.65 | 0.75 |
| 투사체 속도 (px/s) | 350 | 350 | 350 | 350 | 350 |
| 공격 속성 | PHYSICAL | PHYSICAL | PHYSICAL | PHYSICAL | PHYSICAL |
...
| 단계 | Lv.1→2 | Lv.2→3 | Lv.3→4 | Lv.4→5 | Lv.5 |
|------|--------|--------|--------|--------|------|
| 비용 | 200 | 600 | 1800 | 3500 | 최대 레벨 |
| 시간 | 9s | 10s | 11s | 12s | — |

**타겟팅 우선순위:**

BALLISTA는 일반 타워와 달리 특정 적 유형을 우선 조준한다.

```
우선 1순위: ARCHER (원거리 물량 압박 대응)
우선 2순위: MAGE (원거리 마법 공격 대응)
fallback:   사거리 내 가장 가까운 일반 적
```

우선 타겟이 사거리 내에 없으면 다음 순위로 내려가며, 두 우선 타겟 모두 없으면 가장 가까운 적을 타격한다.

- 장거리(최대 7타일) 단일 타격 타워. 발사 속도가 느린 대신 단발 공격력이 높다.
- PHYSICAL 공격이므로 HERO 방어구(WARRIOR/MAGE)에 0.6배 감소가 적용된다. ARCHER/TANKER(PHYSICAL 방어구)에도 0.7배 감소.
- UNARMORED 적(CITIZEN)에 1.3배로 효과적이나, 등장 시점과 맞지 않아 실질적으로 ARCHER 우선 타겟팅이 핵심 운용 방식이다.
- THORN과 달리 유도(homing) 기능이 없다. 투사체는 발사 방향으로 직선 이동한다.
- SPORE/THORN의 근~중거리 역할을 보완하는 원거리 저격 포지션이다.
...
| 항목 | 값 |
|------|----|
| 기본 생산량 (baseAmount) | 14 자원/회 |
| 생산 간격 (interval) | 5초 |

> **Phase 6 변경:** 기본 생산량이 15 → 7 → 14로 조정되었다. 초기 50% 감소 후 2배 증가하여 최종 14로 확정.

**레벨별 생산량 — 공식: `Math.round(baseAmount × (1 + (level - 1) × 0.3))`**

| 레벨 | 생산량 (자원/회) | 계산 |
|------|----------------|------|
| Lv.1 | 14 | 14 × 1.0 |
| Lv.2 | 18 | Math.round(14 × 1.3) |
| Lv.3 | 22 | Math.round(14 × 1.6) |
| Lv.4 | 27 | Math.round(14 × 1.9) |
| Lv.5 | 31 | Math.round(14 × 2.2) |

생산량은 `Math.round()`로 정수 반올림 처리된다. 광고 버프(G.adBuff) 활성 시 생산량이 2배가 된다.
...
**NEST 레벨별 자원 건물 배치 제한 (NEST_BUILD_CAP):**

NEST 레벨에 따라 배치 가능한 건물 수(NEST 자체 제외)가 제한된다.
...
> **Phase 6 변경:** [10, 20, 50] → [5, 10, 25]로 50% 감소. 건물 배치 전략의 중요성을 높이기 위한 조정.

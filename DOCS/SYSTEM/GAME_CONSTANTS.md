...
> **카테고리:** SYSTEM
> **최초 작성:** 2026-03-23
> **최종 갱신:** 2026-03-23 (Phase 7: ARCHER 추가, ENEMY_CAP, SPAWN_SCHEDULE 14구간 확장, GAME_DURATION 900s, 버튼 위치 재배치)
...
| 버튼 | CSS 위치 | 크기 | Portrait 모드 크기 |
|------|----------|------|-------------------|
| 건설 트리거 (`#build-trigger-btn`) | right:12px, bottom:12px | min-width:80px, min-height:48px | min-height:40px |
| 광고 버프 (`#ad-buff-btn`) | left:12px, bottom:12px | 44x44px | 38x38px |
| 줌 컨트롤 (`#zoom-controls`) | right:12px, top:50px | 48x48px (각 버튼) | 40x40px |

> **변경 전:** 건설 버튼=중앙하단, 광고 버프=좌상단, 줌=우측중앙.
> **변경 후:** 건설 버튼=우하단, 광고 버프=좌하단, 줌=우상단. 하단 양쪽 모서리에 주요 액션 버튼을 배치하여 엄지 접근성을 높이고, 줌은 자주 쓰지 않으므로 우상단으로 이동시켰다.
...
| 요소 | Portrait 조정 내용 |
|------|-------------------|
| `#hud-top` | padding: 4px 8px, font-size: 11px |
| `.hud-label` | font-size: 9px |
| `.hud-value` | font-size: 12px |
| `#building-panel` | max-height: 40vh, overflow-y: auto |
| `.bp-btn` | min-height: 30px, font-size: 11px |
| `#build-trigger-btn` | min-height: 40px, font-size: 12px |
| `#zoom-controls button` | 40x40px |
| `#ad-buff-btn` | 38x38px |
